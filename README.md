...
一个面向个人创作的本地 AI 叙事与角色扮演引擎。它把剧本内容包、角色卡、世界书、Prompt 预设、分层记忆、世界状态和多 Provider 调用组织成一条可审阅、可回退的故事生产流程。

> 从剧本书架选定世界，在开局向导中塑造主角与天命，进入第一幕；系统在后续对话中维护人物关系、事实、事件账本和长期记忆。

| 发布状态 | 运行方式 | 数据位置 | 兼容原则 |
| --- | --- | --- | --- |
| `v0.5.0` 稳定自用版 | Node.js 20+ 本地网页 | 角色卡、世界书、密钥与存档仅保存在本机 | 兼容酒馆资源格式和常见交互语义，不运行未知第三方代码 |

## 你可以用它做什么

- 从内置或自定义剧本建立独立故事工程，绑定自己的角色卡、世界书、预设、主题和舞台背景。
- 在创作模式中审阅设定、资源冲突、Prompt 装配和记忆状态，在沉浸模式中专注阅读与续写。
- 使用 Edit、重生成和 Swipe 管理故事分支；用事件账本、世界状态、章节摘要和事实卡维持长篇连续性。
- 接入 OpenAI-compatible、DeepSeek、Gemini、Anthropic 等 Provider，并为不同叙事职责选择不同模型。
- 导入 Character Card V2、SillyTavern Lorebook、Prompt 预设和常见轻前端资源，先评定、再入库、后组装。

本项目不是社区资源镜像，也不复刻 SillyTavern 的第三方 JavaScript 运行环境。动态脚本、DOM、iframe 和未知网络调用会被阻断；可安全保留的静态内容会明确标记为“安全降级”，不会静默丢失语义。

## 快速开始（3 步）

1. 启动本地服务：首次运行执行 `npm install` 和 `npm test`，然后执行 `npm start`，浏览器打开 `http://127.0.0.1:5178`。
2. 配置 Provider：在空会话封面点击“去配置”，选择厂家、填写 API Key 和模型，并先执行“测试连接”。
3. 选择剧本并入局：打开“剧本书架”，选择基础剧本或自定义剧本，完成主角与天命设置后进入第一幕。

所有会话、导入素材和 Provider 配置均保存在本机。API Key 不会通过 `/api/state` 明文返回，也不会进入 Git。
...
| 自动化测试 | `440 / 440` 通过 |
| 数据迁移 | schema `v3` |
| 本地健康接口 | `0.5.0 / stable-local` |
| 仓库内容边界 | 通过，不包含第三方角色卡、世界书、Prompt、立绘或个人存档 |

兼容评定只有三种正式结论：

- **完整映射**：登记样例的主要行为与原资源一致，自动化断言通过。
- **安全降级**：保留静态内容，禁用不安全部分，并在导入预览中列出差异。
- **阻断运行**：资源依赖动态脚本、DOM、iframe、未知网络调用或超出预算的递归，允许审阅但拒绝进入运行流程。
...
冻结范围与重点回归清单见 [`docs/release-v0.5.0.md`](docs/release-v0.5.0.md)，正式兼容原则见 [`docs/tavern-compatibility-policy-v1.md`](docs/tavern-compatibility-policy-v1.md)。
...
- `OpenAI Compatible`：适用于 OpenAI、DeepSeek、OpenRouter、硅基流动、本地 Ollama/LM Studio 以及其他兼容服务。
- `Anthropic`：使用 Claude Messages API。
- `Gemini`：使用 Google Gemini `generateContent` / 流式接口。

界面会根据厂家提供常用模型候选，也允许填写自定义模型。建议保存前先使用“测试连接”；不同 Provider 的额度、内容策略和模型能力由对应服务商决定。
