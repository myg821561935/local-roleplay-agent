...
`v0.5.0-rc.1` 是当前冻结的轻前端兼容候选版。近期测试应以该版本为基线记录 Provider、剧本、会话与复现步骤；个人导入素材、故事工程和角色图片仍只保存在本机，不进入 git。
...
冻结范围与重点回归清单见 `docs/release-v0.5.0-rc.1.md`。

## v0.5 轻前端兼容（RC）

- 社区卡中的安全显示正则可随自定义剧本进入会话，只修改渲染副本，不污染原始消息、记忆和事件账本。
- 纯文本 Quick Reply 与 `/send`、`/say` 会映射到原生输入栏；脚本命令链继续禁用。
...
- 酒馆助手和小白 X 命名空间中的变量、显示正则、文本按钮和声明式面板可安全映射；其脚本生命周期仍标为缺失。
...
- 导入兼容报告会分别显示“可安全保存”和“可直接游玩”，避免把“能导入”误写成“完全兼容”。

当前安全边界与数据结构见 `docs/light-frontend-runtime-v1.md`。
