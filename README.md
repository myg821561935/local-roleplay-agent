...
1. 启动本地服务：执行 `npm run start:local`，浏览器打开 `http://127.0.0.1:5178`。
...
- `用于新剧本`：把角色卡或世界书带入独立的自定义剧本流程。
- `高级拼装`：进入剧本工坊，组合题材基线、角色、世界书和 Prompt。
...
`v0.4.1-rc.1` 是当前冻结的稳定自用候选版。近期测试应以该版本为基线记录 Provider、剧本、会话与复现步骤；个人导入素材、故事工程和角色图片仍只保存在本机，不进入 git。

冻结范围与重点回归清单见 `docs/release-v0.4.1-rc.1.md`。
